...
$diamonds_api = importEtherealDiamonds($vendorArr);
...
foreach($diamonds_api as $diamonds_feed) {
    ...
    if($k < 10000){
    $Sku                    = $diamonds_feed['STNO'];
    $Style_certificate_num  = $diamonds_feed['CERT_NO'];
    ...
    $DepthPct               = number_format($diamonds_feed['DepthPercent'],1);
    $TablePct               = number_format($diamonds_feed['TablePercent'],1);
    $Fluorescence           = GetFluorescenceValue($diamonds_feed['Fluorescence']);
    //$LWRatio                = $diamonds_feed['LWRation'];
    $CertLink               = "https://v360.serveftp.net/certificates/".$Style_certificate_num.".pdf";
    $VideoLink              = addhttps($diamonds_feed['DiamondVideo']);
    
    $image_Link              = addhttps($diamonds_feed['DiamondImage']);

    $WholesalePrice         = $diamonds_feed['PriceTotal'];
    
   
    $Measurements           =   $diamonds_feed['Measure'];
    ...
    $PriceCt                = $diamonds_feed['PricePerCarat'];        // PricePerCt


    // Measurements
    if(!empty($Measurements)){
        if(strpos($Measurements, 'X')!==false){
    ...
    // Culet 
    $Culet=isset($diamonds_feed['Culet'])?$diamonds_feed['Culet']:'None';
    // Girdle
