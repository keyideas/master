<?php 
$csvFolderpath = import_BareDiamond['csv_file_path'];;
...
$filters = Filters;
$vendor_code = import_BareDiamond['vendor_code'];
	...
	if((file_exists($csvfile))) {
		...
		while(! feof($file_handle)) {
			...
			if(!empty($Columnsdata)){
				$diamond = array_combine($header, $Columnsdata);
				if($i>0 && !empty($diamond['Cert#']) && !empty($diamond['Price'])) {
					$RefNo					=	$diamond['Stock#'];  				    // Lot#
					...
					$Style_certificate_num	=	$diamond['Cert#'];					// CertificateNo
					$Length					=	$diamond['L-mm'];					// Length
					$Width					=	$diamond['W-mm'];					// Width
					$Depth					=	$diamond['D-mm'];					// Depth
					...
					$Desc					=	$diamond['Remarks'];					// Desc
					$Location				=	$diamond['County of Orig'];					// Origin
					$PriceCt				=	$diamond['Price'];				// PricePerCt
					$CertLink				=	addHttps($diamond['Cert file']);	// Certificate URL
					$VideoPath				=	$diamond['Video URL'];					// Video Link
					$ImagePath				=	$diamond['Image URL'];					// image Link
					// Measurements 
					...
					if(!empty($RefNo)){
						...
						$Sku=generateSku(import_BareDiamond['type']);
					}
					//Stock Number
					$stockNumber=generateStockNumber($Style_certificate_num,import_BareDiamond['type']);
					...
					if($SizeCt < 1){
						$admin_margin = ($diamond_price)*(import_BareDiamond['onect_below_margin_price']/100);
					}
					elseif($SizeCt >= 1){
						$admin_margin = ($diamond_price)*(import_BareDiamond['onect_above_margin_price']/100);
					}
					
					$PropCode = changeCutGrade($PropCode,$PolishName,$SymName,$shape_cat,$CertType);
					$product_title_raw=make_diamond_name($SizeCt,$Color,$Clarity,$PropCode,$shape_cat,$stockNumber);
					$product_title=trim(str_replace($stockNumber, '', $product_title_raw));
					$product_name=make_diamond_seo_url($SizeCt,$Color,$Clarity,$PropCode,$shape_cat,$stockNumber);
					$description=make_diamond_description($SizeCt,$Color,$Clarity,$PropCode,$shape_cat,$stockNumber,$CertType,import_BareDiamond['type']);
					$ImagePath = ($ImagePath == "") ? make_diamond_image($VideoPath) : $ImagePath;
					
					$isFilterValidate = filterValidation($CertType,$shape_cat,$Color,$Clarity,$PropCode,$SizeCt,$PolishName,$SymName,$diamond_price,$ImagePath,$VideoPath,trim($PriceCt),trim($Style_certificate_num),$FLName,$CertLink,$filters,$DepthPer,$TablePer);
		            if(!$isFilterValidate){
		                 $i++;
		                continue;
		            }
