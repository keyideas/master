...
function filterValidation($CertType,$ShapeCode,$Color,$Clarity,$Cut,$SizeCt,$Polish,$Symmetry,$WholesalePrice,$Image_Url,$Video_Url,$PriceCt,$Style_certificate_num,$Fluorescence,$CertLink,$filters){
    ...
    if(!isset($Image_Url) && !isset($Video_Url) && !isset($CertLink)){
        echo 'Empty'.$Video_Url.'\n';
        return 0;
    }
 ...
 if ( ! function_exists( 'make_diamond_image' ) ) {
    function make_diamond_image($url = '') {
        ...
        if($url != ''){
            ...
            $diamond_id = $params['d'];
			if((strpos($url, "v3601300.v360.in/vision360.html?") !== FALSE)){
 ...
 function getSegomaImage($url='',$stockNumber='',$certificate='')
{
   ...
   if ($url!='' && ((($pos = strpos($url, "v360.in/diamondview.aspx?cid=IPMDE&d=")) !== FALSE))) { 
    ...
    elseif ($url!='' && strpos($url, "v360.in/diamondview.aspx?cid=APShaps") !== FALSE) { 
        ...
        $vendor_array = ['lg2'=>55, 'lg4'=>103, 'diamond360'=>44, 'Diamond360'=>44, 'APShaps'=>99, 'forever'=>59, 'IPMDE'=>289];
        $server_array = ['lg2'=>'s4', 'lg4'=>'s4', 'diamond360'=>'s6', 'Diamond360'=>'s6', 'APShaps'=>'s4', 'forever'=>'s4', 'IPMDE'=>'s4'];
        $url_components = parse_url($url); 
        parse_str($url_components['query'], $params);
    ...
    elseif ((strpos($url, "https://v360.in/LG4/vision360.html") !== FALSE) || (strpos($url, "https://v360.in/LG3/vision360.html") !== FALSE) || (strpos($url, "https://v360.in/LG2/vision360.html") !== FALSE) || (strpos($url, "https://v360.in/LG1/vision360.html") !== FALSE)) {
        $vendorName = 'lg3';
        ...
        $vendor_array = ['lg2'=>55, 'lg4'=>103, 'lg3'=>107, 'lg1'=>57];
        $url_components = parse_url($url); 
        parse_str($url_components['query'], $params);
        if( isset($params['d']))
        {
            $imageUrl = "https://s4.v360.in/images/company/".$vendor_array[$vendorName]."/imaged/".$params['d']."/still.jpg";
        }
        ...
        $imageUrl = "";
    }elseif ((strpos($url, "sanghvisons.com/worldproductdetails/index/") !== FALSE)) {
    ...
    function getVideoUrl($videoUrl='',$stockNumber='')
    {
        if($videoUrl!=''){
            ...
            $videoUrl = "https://s3.ap-south-1.amazonaws.com/mediadiam/VideoNew/Vision360.html?d=".$param;
                ...
                $videoUrl = "";
            }elseif ((strpos($videoUrl, "sanghvisons.com/worldproductdetails/index/") !== FALSE)) {
...
if ( ! function_exists( 'defaultVideoFormat' ) ) {
    ...
    {
        $videoImageUrl = [];
        if(strpos($url, "https://api1.v360.in/") !== FALSE){
...
if ( ! function_exists( 'moveToArchives' ) ) {
    function moveToArchives($folderpath,$csvName)
    {
        ...
        if (!@copy($srcPath,$ak)) {
            ...
            //unlink($srcPath);
            $files = glob($folderpath . "{*.csv,*.CSV}", GLOB_BRACE);
...
if ( ! function_exists( 'generateStockNumber' ) ) {
    ...
    {
        global $wpdb;
        $rowid = $wpdb->get_results("SELECT id FROM ".$wpdb->prefix."custom_kdmdiamonds WHERE Style='".$certNum."'");
        if($rowid){
...
if ( ! function_exists( 'changeCutGrade' ) ) {
    function changeCutGrade($cut='',$polish='',$symmetry='',$shape='',$CertType='') {
        ...
        if($cut == '' || $cut == '-')
        {
            if(($polish == 'Excellent') && ($symmetry == 'Excellent')){
                $cut    =   'Ideal';
            }
            else if((($polish == 'Ideal') || ($polish == 'Excellent')) && (($symmetry == 'Ideal') || ($symmetry == 'Excellent'))){
                $cut    =   'Ideal';
            }else if((($polish == 'Very Good') || ($polish == 'Ideal') || ($polish == 'Good') || ($polish == 'Excellent')) && (($symmetry == 'Very Good') || ($symmetry == 'Excellent') || ($symmetry == 'Good') || ($symmetry == 'Ideal'))){
                $cut    =   'Excellent';
            }else{
                 $cut    =   'Ideal';
            }
...
function importExcellentDiamonds($arr){
    ...
    $curl = curl_init();
    curl_setopt_array($curl, array(
      CURLOPT_URL => "http://webapi.excellentdiamonds.com.hk/DeveloperAPI.svc/Full_Stock_List?username=".$arr['username']."&password=".$arr['password']."",
      CURLOPT_RETURNTRANSFER => true,
      CURLOPT_ENCODING => "",
      CURLOPT_MAXREDIRS => 10,
      CURLOPT_TIMEOUT => 0,
